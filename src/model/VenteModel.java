 ...
 * @author Nsat04
 */
class VenteModel  extends  ModelFactory implements ModelInterface<Vente>{
  ArrayList<Vente>listevente=new ArrayList<>();
    ...
    public Vente find(int id) {
        ...
        try {
            ...
            while (this.resultSet.next()) {                
                ...
                vente.setQuantiteRestante(this.resultSet.getInt("qte_restante"));
    ...
    public boolean insert(Vente table) {
        try {
            ...
            this.preparedStatement=Database.statement("INSERT INTO vente SET id=?, date_vente=?, qte_vendu=?, qte_restant=?,prix_vente=?, product_id=?");
            ...
            this.preparedStatement.setInt(4, table.getQuantiteRestante()); 
             this.preparedStatement.setDouble(5, table.getPrixVente()); 
              this.preparedStatement.setInt(6, table.getProduct().getId()); 
            this.preparedStatement.execute();
    ...
    public boolean update(Vente table) {
         try {
            ...
            this.preparedStatement=Database.statement("UPDATE  vente SET id=?, date_vente=?, qte_vendu=?, qte_restant=?,prix_vente=?, product_id=? WHERE id=?");
            this.preparedStatement.setInt(1, table.getId());
            this.preparedStatement.setDate(2, java.sql.Date.valueOf(table.getDateVente()));
            this.preparedStatement.setInt(3, table.getQuantiteVendue());
            this.preparedStatement.setInt(4, table.getQuantiteRestante()); 
            this.preparedStatement.setDouble(5, table.getPrixVente()); 
            this.preparedStatement.setInt(6, table.getProduct().getId()); 
            this.preparedStatement.setInt(7, table.getId());
            this.preparedStatement.execute();
